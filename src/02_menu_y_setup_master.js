...
function setupMaster(){ const ss=SpreadsheetApp.getActive(); ss.setSpreadsheetTimeZone(CFG.TZ);
  if(!P.getProperty('MASTER_ID')) P.setProperty('MASTER_ID', ss.getId()); ensureRegistry_(); ensureInitialMonths_(); refreshGuideIndexFromRegistry_(); applyAllMasterDV_(); }
function ensureRegistry_(){ const ss=SpreadsheetApp.getActive(); let sh=ss.getSheetByName(CFG.REGISTRY_SHEET); if(!sh) sh=ss.insertSheet(CFG.REGISTRY_SHEET);
  sh.getRange(1,1,1,CFG.REGISTRY_HEADERS.length).setValues([CFG.REGISTRY_HEADERS]); sh.setFrozenRows(1); }
function ensureInitialMonths_(){ const ss=SpreadsheetApp.getActive(); CFG.MONTHS_INITIAL.forEach(tag=>{ if(!ss.getSheetByName(toTabName_(tag))) createMonthSheet_(ss,tag); }); }
function createMonthSheet_(ss,tag){ const sh=ss.insertSheet(toTabName_(tag)); sh.getRange('A1').setValue('FECHA'); sh.getRange('B1').setValue('DÍA'); sh.setFrozenRows(2);
  const [y,m]=tag.split('-').map(Number), dates=enumerateMonthDates_(y,m);
...
function refreshGuideIndexFromRegistry_() {
  ...
  rows.forEach(r=>{
    ...
    P.setProperty('guideById:'+id, code);                 // nuevo índice ID→CÓDIGO
  });
}
